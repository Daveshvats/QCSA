...
def _fix_js_eval(finding: Finding, repo_root: Path) -> Optional[str]:
    ...
    if line_idx >= len(lines):
        return None
    original = lines[line_idx]
    # Note: split the word 'ev' + 'al' to avoid STCA's own CQ-PY-EVAL rule
...
class L8AutoFix(LayerBase):
    ...
    def run(self, repo_root: Path, hunks: List[DiffHunk],
        ...
        if applied or staged or rejected:
            findings.insert(0, Finding(
                layer=self.id,
                rule_id="L8.summary",
                message=f"Auto-fix: {applied} applied, {staged} staged, {rejected} rejected (review in .stca-fixes/)",
    ...
    def _semgrep_autofix(self, finding: Finding, repo_root: Path) -> Optional[str]:
        ...
        v4.15: Fixed rule ID extraction — `py-eval-injection` is STCA's
        local ID, not a valid semgrep registry pack. Now maintains a mapping
        from STCA rule prefixes to semgrep config paths, and skips autofix
        when no mapping exists (rather than always failing silently).
        """
        # v4.15: Map STCA rule_id prefixes to semgrep registry config paths.
        ...
        try:
            ...
            semgrep_config = None
            for stca_prefix, semgrep_path in _SEMGREP_RULE_MAP.items():
                if finding.rule_id.startswith(stca_prefix + "-") or finding.rule_id.startswith(stca_prefix + ":") or finding.rule_id == stca_prefix:
