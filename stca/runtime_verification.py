...
How it works:
  ...
  2. STCA instruments the code (AST transformation) to check the invariant
...
def find_invariant_annotations(file_path: Path) -> List[Dict[str, Any]]:
    ...
    Looks for:
      ...
      @stca.invariant("self.state in ['active', 'pending']")
...
def provide_invariant_decorator() -> str:
    ...
    Users add this to their code or import it from stca.
    """
    return textwrap.dedent("""
# STCA Runtime Verification — @invariant decorator
# Usage:
...
#           self.balance -= amount
#
# STCA will instrument this to check the invariant after each method call.

def invariant(expr):
    \"\"\"Decorator that marks a class/function for runtime invariant checking.

    STCA instruments the code to check this invariant after each
    state-modifying operation.
    \"\"\"
    def decorator(target):
        # Store the invariant expression on the target
        if not hasattr(target, '_stca_invariants'):
            target._stca_invariants = []
        target._stca_invariants.append(expr)
        return target
    return decorator
""").strip()
