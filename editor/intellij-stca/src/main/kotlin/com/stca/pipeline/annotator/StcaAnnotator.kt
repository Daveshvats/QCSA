package com.stca.pipeline.annotator
...
/**
 * External annotator that surfaces STCA findings as squiggles.
 *
 ...
 */
class StcaAnnotator : ExternalAnnotator<List<Finding>, List<Finding>>() {
    ...
    override fun collectInformation(file: PsiFile): List<Finding>? {
        // Run `stca check --full --json` and parse findings
    ...
    override fun apply(file: PsiFile, annotationResult: List<Finding>, holder: AnnotationHolder) {
        for (finding in annotationResult) {
            ...
            holder.newAnnotation(
                ...
                "[STCA ${finding.ruleId}] ${finding.message}"
            ).range(range).create()
        }
    }
}
