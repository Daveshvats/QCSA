...
so LoomScan works out-of-the-box without manual setup.

Strategy:
  - Python tools (mutmut, atheris, pip-audit): pip-install into LoomScan's venv
  - Go binaries (gitleaks, semgrep, osv-scanner): download pre-compiled
    binaries from GitHub releases into ~/.loomscan/bin/ with SHA256 verification
...
STCA_HOME = Path(os.environ.get("STCA_HOME", Path.home() / ".loomscan"))
...
def get_loomscan_bin_dir() -> Path:
    """Return the LoomScan binary directory, creating it if needed."""
    BIN_DIR.mkdir(parents=True, exist_ok=True)
    return BIN_DIR


def ensure_loomscan_on_path() -> None:
    """Ensure LoomScan's bin dir is on PATH for subprocess calls."""
    bin_dir = str(get_loomscan_bin_dir())
    if bin_dir not in os.environ.get("PATH", "").split(os.pathsep):
        os.environ["PATH"] = bin_dir + os.pathsep + os.environ.get("PATH", "")


def is_tool_installed(name: str) -> bool:
    """Check if a tool is available (either on PATH or in LoomScan bin dir)."""
    ensure_loomscan_on_path()
...
def download_with_progress(url: str, dest: Path, timeout: int = 60) -> bool:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "loomscan-installer/0.1"})
...
def print_install_report(results: Dict[str, Tuple[bool, str]]) -> None:
    ...
    print()
    print("=" * 60)
    print("LoomScan Tool Installation Report")
    print("=" * 60)
    for name, (ok, msg) in results.items():
        status = "✓" if ok else "✗"
        print(f"  {status} {name:<20} {msg}")
    print()
    print(f"LoomScan bin dir: {BIN_DIR}")
