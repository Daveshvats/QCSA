...
Language scope:
  ...
  which would break LoomScan's "works on any laptop, offline" design.
...
Cross-platform support (for Python):
  ...
  - Windows/macOS/Linux:  Uses built-in coverage-guided fuzzer
    (loomscan.fuzz_coverage — pure Python, sys.monitoring BRANCH events)
...
class L4Fuzz(LayerBase):
    ...
    def _generate_harness(self, repo_root: Path, hunk: DiffHunk,
        ...
        harness_dir = repo_root / ".loomscan-cache" / "fuzz"
    ...
    def _generate_coverage_harness(self, module_path: str, func_name: str) -> str:
        ...
        Uses loomscan.fuzz_coverage which implements the same algorithm as
        ...
        return textwrap.dedent(f"""
            ...
            # Ensure the loomscan package is importable. We add multiple paths:
            # 1. The repo root (so the target module imports work)
            # 2. The loomscan package location (found via the harness file's location)
            _harness_dir = os.path.dirname(os.path.abspath(__file__))
            _repo_root = os.path.dirname(os.path.dirname(_harness_dir))
            sys.path.insert(0, _repo_root)
            # Also add the loomscan package parent if loomscan is installed elsewhere
            try:
                import loomscan
                _loomscan_parent = os.path.dirname(os.path.dirname(os.path.dirname(loomscan.__file__)))
                if _loomscan_parent not in sys.path:
                    sys.path.insert(0, _loomscan_parent)
            except ImportError:
                pass
    ...
    def _generate_random_harness(self, module_path: str, func_name: str) -> str:
        ...
        return textwrap.dedent(f"""
            ...
            from loomscan.fuzz_fallback import FuzzedDataProvider, fuzz
    ...
    def _run_harness(self, harness_path: Path, repo_root: Path,
        ...
        try:
            ...
            # 1. The loomscan package (for fuzz_coverage/fuzz_fallback imports)
            # 2. The repo_root (for the target module imports)
            import os
            env = os.environ.copy()
            loomscan_parent = str(Path(__import__("loomscan").__file__).resolve().parent.parent)
            existing_path = env.get("PYTHONPATH", "")
            env["PYTHONPATH"] = os.pathsep.join(
                p for p in [str(repo_root), loomscan_parent, existing_path] if p
            )
            ...
            if proc.returncode != 0:
                ...
                # for loomscan means the env isn't set up, not a target bug)
                combined = proc.stdout + proc.stderr
                if "ModuleNotFoundError: No module named 'loomscan'" in combined:
