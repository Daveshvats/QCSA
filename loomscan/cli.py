"""LoomScan CLI — `loomscan init|check|bootstrap|report|feedback`."""
...
def init(repo: str, force: bool):
    """Create a .loomscan.yaml config file in the repo."""
    repo_root = Path(repo).resolve()
    cfg_path = repo_root / ".loomscan.yaml"
    ...
    click.echo("\nNext steps:")
    click.echo("  1. Run `loomscan install-tools` to auto-install gitleaks, semgrep, opa, etc.")
    click.echo("  2. Run `loomscan bootstrap invariants` to infer invariants from tests")
    click.echo("  3. Run `loomscan check` on a git diff to scan changes")


@main.command()
@click.option("--repo", default=".", help="Repository root")
@click.option("--open-dashboard", is_flag=True, help="Generate and open dashboard after scan")
def quickstart(repo: str, open_dashboard: bool):
    """v5.0: Quick start — create config, run first scan, show next steps.
    ...
    It will:
      1. Create a .loomscan.yaml config (if not present)
      2. Run a full scan
      3. Show a summary of findings
      4. Print next steps
    """
    repo_root = Path(repo).resolve()
    if not (repo_root / ".git").exists():
        click.echo(f"Not a git repo: {repo_root}. Run `git init` first.", err=True)
        sys.exit(2)
    ...
    if not cfg_path.exists():
        ...
        click.echo(f"✓ Config exists: {cfg_path}")
    ...
    try:
        progress.__enter__()
        orch = Orchestrator(repo_root, config, strictness=5, progress=progress)
    ...
    click.echo(f"\n{'='*50}")
    click.echo(f"  LoomScan Quick Start — Scan Complete")
    click.echo(f"{'='*50}")
    ...
    click.echo(f"  Decision: {result.final_decision.value}")
    click.echo(f"{'='*50}")

    # Step 4: Next steps
    click.echo(f"\n📋 Next steps:")
    click.echo(f"  1. View detailed findings:  loomscan check --full --summary")
    click.echo(f"  2. View JSON output:        loomscan check --full --json")
    click.echo(f"  3. Generate dashboard:      loomscan dashboard --repo {repo_root}")
    click.echo(f"  4. Run quality gate:        loomscan gate --full --preset balanced")
    click.echo(f"  5. Apply auto-fixes:        loomscan fix --apply")
    click.echo(f"  6. Set strictness:          loomscan strictness --level 7")
...
def install_tools_cmd(force: bool, layers: tuple):
    ...
    opa, trivy) are downloaded from GitHub releases into ~/.loomscan/bin/ with
    SHA256 verification.
    """
    if layers:
...
def rules_list():
    ...
    click.echo("\nBuilt-in rule packs (ship with LoomScan):\n")
    ...
    click.echo("\nPull an external pack with: loomscan rules pull <name>")
...
def rules_pull(name: str, repo: str):
    ...
    if name not in list_external_packs():
        click.echo(f"Unknown pack: {name}. Run `loomscan rules list` to see options.")
        return
    repo_root = Path(repo).resolve()
    dest_dir = repo_root / ".loomscan-cache"
    manifest = pull_external_pack(name, dest_dir)
    click.echo(f"Pulled external pack '{name}'. Manifest: {manifest}")
    click.echo("The pack will be used automatically on the next `loomscan check`.")
...
def rules_submit(pack: str, name: str, language: str, description: str,
    ...
    """v4.41: Submit a community rule pack for inclusion in LoomScan.

    Validates a YAML rule pack, packages it with metadata, and generates
    a submission-ready file that can be shared with the LoomScan project.
    ...
    Examples:
      loomscan rules submit --pack my-rules.yml --name my-company-security \\
    ...
    if output == "-":
        ...
        click.echo(f"\nTo contribute: submit this file to the LoomScan project.")
        click.echo(f"  GitHub: https://github.com/YOUR_ORG/loomscan/issues")
        click.echo(f"  Or: loomscan rules install {out_path}")
...
def monorepo_cmd(repo: str, add: Optional[str], remove: Optional[str], list_: bool, scan: bool):
    ...
    Configure multiple workspace roots for monorepo scanning. LoomScan will scan
    each workspace as a separate logical project, merging findings with
    workspace-prefixed paths.

    Examples:
      loomscan monorepo --add 'apps/*'             # add a workspace pattern
      loomscan monorepo --add 'packages/*'
      loomscan monorepo --list                     # list resolved workspaces
      loomscan monorepo --scan                     # scan each, report counts
      loomscan monorepo --remove 'apps/*'          # remove a pattern
    """
    repo_root = Path(repo).resolve()
    ...
    if list_:
        if not config.workspaces:
            ...
            click.echo("Add with: loomscan monorepo --add 'apps/*'")
            return
    ...
    if scan:
        if not config.workspaces:
            click.echo("No workspaces configured. Add with: loomscan monorepo --add 'apps/*'")
            return
        resolved = config.resolve_workspaces(repo_root)
    ...
    # Default: show help
    click.echo("LoomScan monorepo management. Use --add, --remove, --list, or --scan.")
    ...
    if fmt == "cyclonedx":
        return json.dumps({
            ...
            "metadata": {
                ...
                "tools": [{"vendor": "LoomScan", "name": "loomscan", "version": __version__}],
            },
        ...
        lines = ["SPDXVersion: SPDX-2.3",
                 ...
                 f"Creator: Tool: loomscan-{__version__}",
                 ""]
...
@click.option("--output", "-o", "output_path", default=None,
              help="Output file path for SARIF report (default: .loomscan-reports/result.sarif). "
...
def check(repo: str, base: str, staged: bool, as_json: bool, quiet: bool,
    ...
    if verbose:
        ...
        _logging.getLogger("loomscan").setLevel(_logging.WARNING)
    ...
    if sarif:
        ...
        if output_path == "-":
            ...
            # Write to file (default: .loomscan-reports/result.sarif to match _save_reports)
            if output_path:
                ...
                sarif_dir = repo_root / ".loomscan-reports"
        ...
        click.echo(f"\n=== LoomScan Summary: {len(result.findings)} findings across {len(by_rule)} rules ===\n")
...
def gate(repo: str, full: bool, base: str, staged: bool, preset: Optional[str],
    ...
    Examples:
      loomscan gate --full                                  # full scan, default gate
      loomscan gate --preset strict --full                  # strict preset
      loomscan gate --preset balanced --full                # balanced preset (default)
      loomscan gate --preset permissive --full               # permissive preset
      loomscan gate --preset custom --max-critical 0 --max-high 5  # custom
      loomscan gate --max-critical 0 --max-high 5           # allow up to 5 high
      loomscan gate --max-density 5.0 --full                # fail if >5 findings/1k LOC
...
@main.command()
@click.option("--repo", default=".", help="Repository root")
@click.option("--apply", is_flag=True, help="Apply patches directly to source files (default: stage in .loomscan-fixes/)")
@click.option("--finding-id", help="Only apply fix for a specific finding fingerprint")
def fix(repo: str, apply: bool, finding_id: str):
    """Apply auto-fixes for findings from the last `loomscan check` run.

    By default, fixes are staged in `.loomscan-fixes/` for review. Use --apply
    to apply them directly to source files.
    """
    repo_root = Path(repo).resolve()
    fixes_dir = repo_root / ".loomscan-fixes"
    if not fixes_dir.exists():
        click.echo("No fixes available. Run `loomscan check` first.")
        return
    ...
    if apply:
        ...
        click.echo("To apply: loomscan fix --apply")
...
def tuning_apply(repo: str):
    ...
    Reads .loomscan-stats.json, computes precision/recall per layer, and writes
    ...
    if not stats:
        click.echo("No feedback stats recorded yet. Run `loomscan feedback tp/fp` first.")
        return
...
def llm_verify_cmd(repo: str, file_path: str, func_name: str):
    ...
    if not config.llm.get("enabled"):
        click.echo("LLM is not enabled. Set llm.enabled: true in .loomscan.yaml and run Ollama.")
        return
...
def precision_mine_history(repo: str, max_commits: int, verify: bool):
    ...
    if not rules:
        ...
        return
    dest = repo_root / ".loomscan-rules" / "mined"
    ...
    click.echo(f"\nSaved to {dest}. These rules will be used automatically on the next `loomscan check`.")
...
def precision_mine_codebase(repo: str, max_files: int):
    ...
    if not rules:
        ...
        return
    dest = repo_root / ".loomscan-rules" / "codebase-mined"
...
def precision_compile_rules(repo: str, file_path: str):
    ...
    config = STCAConfig.from_file(find_config(repo_root))
    if not config.llm.get("enabled"):
        click.echo("LLM is not enabled. Set llm.enabled: true in .loomscan.yaml.")
        return
    llm = LLMClient(endpoint=config.llm.get("endpoint", "http://localhost:11434"),
                    model=config.llm.get("model", "qwen3-coder-1.5b"))
    if not llm.is_available():
    ...
    if not rules:
        ...
        return
    dest = repo_root / ".loomscan-rules" / "llm-compiled"
...
def precision_corroborate(repo: str):
    ...
    if not result_path.exists():
        click.echo("No previous check result found. Run `loomscan check` first.")
        return
    import json
...
def issue_trend(repo: str, weeks: int):
    ...
    if not trend:
        click.echo("No trend data yet. Run `loomscan check` a few times.")
        return
...
def strictness_cmd(repo: str, level: int):
    ...
    if level is None:
        ...
        click.echo(f"\nSet with: loomscan strictness --level N")
        click.echo(f"Use with: loomscan check --strictness N")
    else:
        ...
        # (was causing crash when .loomscan.yaml contained __strictness__)
...
def profile_apply(repo: str, name: str):
    ...
    if not pm.get_profile(name):
        click.echo(f"Profile '{name}' not found. Run `loomscan profile list`.")
        return
    click.echo(f"Profile '{name}' is available. Use: loomscan check --profile {name}")
...
def deadcode_cmd(repo: str, discover: bool):
    ...
    if discover or not analyzer.trace_file.exists():
        ...
        if not analyzer.trace_file.exists():
            ...
            click.echo(f"  1. Instrument: loomscan deadcode instrument")
            click.echo(f"  2. Run tests:  pytest")
            click.echo(f"  3. Report:     loomscan deadcode")
    else:
...
def bootstrap_invariants(repo: str):
    ...
    click.echo(f"Written to {repo_root / '.loomscan-invariants.json'}")
...
def feedback_tp(repo: str, layer_id: str):
    ...
    tracker = StatsTracker(Path(repo).resolve() / ".loomscan-stats.json")
...
def impact(repo: str, changed: tuple):
    ...
    Example:
        loomscan impact --changed loomscan/orchestrator.py --changed loomscan/cpg.py
    """
    repo_root = Path(repo).resolve()
    try:
...
def lsp(repo: str, stdio: bool):
    """Start the LoomScan LSP server for IDE integration (VS Code, Neovim, JetBrains).
