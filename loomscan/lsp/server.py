"""LoomScan LSP Server — real-time squiggles in editors (VS Code, Neovim, JetBrains)."""
...
class LSPServer:
    ...
    def _handle_message(self, message: dict):
        ...
        if method == "initialize":
            self._send_message({"jsonrpc": "2.0", "id": id_, "result": {
                ...
                "serverInfo": {"name": "loomscan-lsp", "version": "1.0.0"}}})
    ...
    def _finding_to_diagnostic(self, finding: dict) -> dict:
        ...
        return {"range": {"start": {"line": max(0, finding.get("line", 1) - 1), "character": 0},
                ...
                "code": finding.get("rule_id", ""), "source": "loomscan", "message": finding.get("message", "")}
    ...
    def _get_hover_info(self, params: dict) -> Optional[dict]:
        ...
        for f in line_findings:
            ...
            msg = f.get("message", "")
            lines.append(f"**LoomScan {sev}** — `{rule_id}`")
            lines.append(f"\n{msg}")
            if f.get("fix"):
                lines.append(f"\n**Fix:** {f['fix']}")
            lines.append("\n---")
    ...
    def _get_code_actions(self, params: dict) -> List[dict]:
        ...
        Each finding with an auto-fix pattern gets a "Apply LoomScan fix" code action.
        ...
        for f in range_findings:
            ...
            if has_fix:
                actions.append({
                    "title": f"LoomScan: Apply fix for {rule_id}",
                    "kind": "quickfix",
                    "command": {
                        "title": "Apply LoomScan auto-fix",
                        "command": "loomscan.applyFix",
                        "arguments": [uri, rule_id, f.get("line", 1)],
                    },
                })
            # Always offer "Show rule details"
            actions.append({
                "title": f"LoomScan: Show details for {rule_id}",
                "kind": "source",
                "command": {
                    "title": "Show LoomScan rule details",
                    "command": "loomscan.showRule",
                    "arguments": [rule_id],
                },
            })
