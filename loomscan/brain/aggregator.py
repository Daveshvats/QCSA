...
class Aggregator:
    ...
    def __init__(self, stats_path: Path = None):
        ...
        self._load_stats()
        # v4.12: Load per-layer tuning adjustments from .loomscan.yaml
        # Previously write-only — loomscan tuning apply wrote them but nothing read them back.
    ...
    def _load_tuning(self):
        ...
        v4.13 read .loomscan-tuning.json but tuner writes .loomscan.yaml.
        v4.14 reads from .loomscan.yaml (same file the tuner writes).
        """
        if not self.stats_path:
            return
        # v4.14: Read from .loomscan.yaml (where tuner.apply_adjustments_to_config writes)
        # NOT .loomscan-tuning.json (which was never written by anything)
        config_path = self.stats_path.parent / ".loomscan.yaml"
