...
SOURCE_PARAM_PATTERNS = [
    ...
    # v5.6: JS/Java/Go source patterns
    "req", "res", "ctx", "context", "message", "event",
    "httpRequest", "httpResponse", "requestBody", "requestParam",
    "queryString", "header", "headers", "cookie", "userInput",
    "command", "cmd", "stdin", "argv", "env",
]
...
SOURCE_CALL_PATTERNS = {
    ...
    # v5.6: JavaScript/Node.js sources
    "req.query": "express_query_params",
    "req.body": "express_request_body",
    "req.params": "express_route_params",
    "req.headers": "express_headers",
    "req.cookies": "express_cookies",
    "req.session": "express_session",
    "process.argv": "node_cli_args",
    "process.env": "node_env_vars",
    "readline": "node_stdin",
    "document.getElementById": "dom_input",
    "document.querySelector": "dom_query",
    "localStorage.getItem": "local_storage",
    "sessionStorage.getItem": "session_storage",
    "location.search": "url_query_string",
    "location.hash": "url_fragment",
    # v5.6: Java sources
    "getParameter": "servlet_param",
    "getHeader": "servlet_header",
    "getCookies": "servlet_cookies",
    "getQueryString": "servlet_query",
    "getRequestURI": "servlet_uri",
    "getInputStream": "servlet_body",
    "getReader": "servlet_reader",
    "System.getenv": "java_env",
    "System.getProperty": "java_system_prop",
    "Scanner.next": "java_scanner_input",
    # v5.6: Go sources
    "r.URL.Query": "go_query_params",
    "r.FormValue": "go_form_value",
    "r.PostFormValue": "go_post_form",
    "r.Header.Get": "go_header",
    "r.Cookie": "go_cookie",
    "r.Body": "go_request_body",
    "os.Getenv": "go_env",
    "os.Args": "go_cli_args",
    "flag.String": "go_flag",
    "bufio.NewReader": "go_stdin",
}
...
SINK_PATTERNS = {
    ...
    # v5.6: JavaScript/Node.js sinks
    "eval": ("CWE-95", "Code injection"),  # already in Python, JS shares this
    "Function": ("CWE-94", "Function constructor RCE"),
    "setTimeout": ("CWE-95", "setTimeout with string — eval injection"),
    "setInterval": ("CWE-95", "setInterval with string — eval injection"),
    "execSync": ("CWE-78", "child_process.execSync — command injection"),
    "execFile": ("CWE-78", "child_process.execFile — command injection"),
    "spawn": ("CWE-78", "child_process.spawn — command injection"),
    "fork": ("CWE-94", "child_process.fork — RCE"),
    "query": ("CWE-89", "SQL query — injection"),
    "innerHTML": ("CWE-79", "XSS — innerHTML"),
    "outerHTML": ("CWE-79", "XSS — outerHTML"),
    "insertAdjacentHTML": ("CWE-79", "XSS — insertAdjacentHTML"),
    "document.write": ("CWE-79", "XSS — document.write"),
    "document.writeln": ("CWE-79", "XSS — document.writeln"),
    "send": ("CWE-79", "XSS — res.send with user input"),
    "write": ("CWE-79", "XSS — res.write with user input"),
    "writeFile": ("CWE-22", "Path traversal — fs.writeFile"),
    "writeFileSync": ("CWE-22", "Path traversal — fs.writeFileSync"),
    "readFile": ("CWE-22", "Path traversal — fs.readFile"),
    "readFileSync": ("CWE-22", "Path traversal — fs.readFileSync"),
    "createReadStream": ("CWE-22", "Path traversal — fs.createReadStream"),
    "fetch": ("CWE-918", "SSRF — fetch with user URL"),
    "request": ("CWE-918", "SSRF — request with user URL"),
    "axios": ("CWE-918", "SSRF — axios with user URL"),
    "log": ("CWE-532", "Secret in log"),
    "error": ("CWE-532", "Secret in log"),  # also console.error
    # v5.6: Java sinks
    "exec": ("CWE-78", "Runtime.exec — command injection"),  # shared with Python
    "getRuntime": ("CWE-78", "Runtime.getRuntime().exec"),
    "ProcessBuilder": ("CWE-78", "ProcessBuilder — command injection"),
    "executeQuery": ("CWE-89", "SQL — Statement.executeQuery"),
    "executeUpdate": ("CWE-89", "SQL — Statement.executeUpdate"),
    "execute": ("CWE-89", "SQL — Statement.execute"),  # shared with Python
    "prepareStatement": ("CWE-89", "SQL — verify parameterization"),
    "createStatement": ("CWE-89", "SQL — raw statement (not prepared)"),
    "forward": ("CWE-22", "RequestDispatcher.forward — path traversal"),
    "sendRedirect": ("CWE-601", "Open redirect — response.sendRedirect"),
    "sendError": ("CWE-79", "XSS — response.sendError"),
    "setHeader": ("CWE-113", "Header injection — response.setHeader"),
    "addHeader": ("CWE-113", "Header injection — response.addHeader"),
    "getWriter": ("CWE-79", "XSS — response.getWriter().write"),
    "getOutputStream": ("CWE-79", "XSS — response.getOutputStream"),
    "readObject": ("CWE-502", "Deserialization — ObjectInputStream.readObject"),
    "evaluate": ("CWE-94", "ScriptEngine.evaluate — RCE"),
    "lookup": ("CWE-74", "JNDI injection — Context.lookup"),
    "println": ("CWE-532", "Secret in log — System.out.println"),
    "printf": ("CWE-532", "Secret in log — System.out.printf"),
    # v5.6: Go sinks
    "Exec": ("CWE-78", "os/exec — command injection"),
    "Command": ("CWE-78", "exec.Command — command injection"),
    "Query": ("CWE-89", "SQL — db.Query"),
    "QueryRow": ("CWE-89", "SQL — db.QueryRow"),
    "Exec": ("CWE-89", "SQL — db.Exec (also command injection)"),
    "Open": ("CWE-22", "Path traversal — os.Open"),
    "OpenFile": ("CWE-22", "Path traversal — os.OpenFile"),
    "ReadFile": ("CWE-22", "Path traversal — os.ReadFile"),
    "WriteFile": ("CWE-22", "Path traversal — os.WriteFile"),
    "Get": ("CWE-918", "SSRF — http.Get"),
    "Post": ("CWE-918", "SSRF — http.Post"),
    "NewRequest": ("CWE-918", "SSRF — http.NewRequest"),
    "Fprintln": ("CWE-532", "Secret in log — fmt.Fprintln"),
    "Printf": ("CWE-532", "Secret in log — fmt.Printf"),
    "Println": ("CWE-532", "Secret in log — fmt.Println"),
    "HTML": ("CWE-79", "XSS — template.HTML bypass"),
    "Text": ("CWE-79", "XSS — template.Text"),
    "Redirect": ("CWE-601", "Open redirect — http.Redirect"),
    "Error": ("CWE-79", "XSS — http.Error"),
}
...
SANITIZER_PATTERNS = {
    ...
    "urllib.parse.quote",
    # v5.6: JavaScript sanitizers
    "encodeURIComponent",  # URL encoding
    "encodeURI",
    "escapeHtml", "escapeHTML",  # HTML escape (JS)
    "sanitize", "DOMPurify.sanitize",  # DOM sanitization
    "parseInt", "parseFloat",  # type conversion
    "Number", "String", "Boolean",
    "JSON.stringify",  # safe for data context
    # v5.6: Java sanitizers
    "escapeHtml", "escapeHtml4", "escapeHtml3",  # Apache Commons Text
    "escapeXml", "escapeXml10", "escapeXml11",
    "escapeJava", "escapeEcmaScript",
    "URLEncoder.encode",  # URL encoding
    "URLDecoder.decode",
    "Integer.parseInt", "Long.parseLong", "Double.parseDouble",
    "PreparedStatement",  # prepared statement (safe SQL)
    "setString", "setInt", "setLong", "setDouble",  # parameter binding
    # v5.6: Go sanitizers
    "QueryEscape", "PathEscape",  # url.QueryEscape, url.PathEscape
    "HTMLescape", "HTMLEscape",  # html.EscapeString
    "Atoi", "ParseInt", "ParseFloat",  # type conversion
    "Prepare",  # prepared statement (db.Prepare)
    "Quote",  # strconv.Quote
}
