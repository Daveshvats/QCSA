...
logger = logging.getLogger("loomscan.orchestrator")


def _max_files(default: int) -> int:
    """Return the effective per-engine file cap.

    v4.33: Reads STCA_MAX_FILES_OVERRIDE (set by `loomscan check --max-files N`).
...
class Orchestrator:
    ...
    def __init__(self, repo_root: Path, config: Optional[STCAConfig] = None,
        ...
        # v5.7: Optional ScanProgress instance for TUI mascot + progress bar.
        # When None, the orchestrator runs silently (backwards-compatible).
        # When set, run() / run_full() call start_stage() / complete_stage()
        # at known boundaries so users see live progress.
        ...
        if self.config.brain.get("enable_project_tuner", False):
            try:
                ...
                tuner_path = self.repo_root / ".loomscan-project-tuner.json"
    ...
    # ---------- v5.7: progress hooks (no-op when progress is None) ----------

    def _stage_start(self, name: str, description: str = "",
                     animate_mascot: bool = True) -> None:
        """Notify the progress tracker that a stage has started (no-op if None)."""
        if self.progress is not None:
            try:
                self.progress.start_stage(name, description, animate_mascot=animate_mascot)
            except Exception:
                pass  # TUI failures must never break the pipeline

    def _stage_complete(self, findings_count: int = 0) -> None:
        """Notify the progress tracker that the current stage is done."""
        if self.progress is not None:
            try:
                self.progress.complete_stage(findings_count=findings_count)
            except Exception:
                pass

    def _stage_fail(self, error: str = "") -> None:
        """Notify the progress tracker that the current stage failed."""
        if self.progress is not None:
            try:
                self.progress.fail_stage(error=error)
            except Exception:
                pass
    ...
    def run_full(self) -> PipelineResult:
        ...
        self._stage_complete(findings_count=len(result.findings) - n_layer_findings_before)
        ...
        # Advanced detection
        # v5.7: progress — stage 2 of ~12 (advanced detection engines)
        self._stage_start("Taint & CPG", "Cross-file taint + CPG queries",
                          animate_mascot=False)
        ...
        self._stage_complete(findings_count=len(result.findings) - n_before)

        # v5.7: progress — stage 3 (research engines)
        self._stage_start("Research Engines", "Metamorphic + differential + coverage",
                          animate_mascot=False)
        n_before = len(result.findings)
        ...
        # v5.7: progress — stage 4 (v2 analyzers)
        self._stage_start("Multi-lang", "v2 analyzers (multi-lang, code quality, AST)",
                          animate_mascot=False)
        n_before = len(result.findings)
        ...
        # v5.7: progress — stage 5 (precision + counterfactual)
        self._stage_start("Precision", "FP learning + counterfactual mutation",
                          animate_mascot=False)
        # Precision pipeline
        ...
        self._stage_complete(findings_count=0)
        ...
        # v5.7: progress — stage 6 (FIS aggregation + LLM)
        self._stage_start("FIS Brain", "Type-2 fuzzy inference + LLM tie-breaker",
                          animate_mascot=False)
        # FIS aggregation
        ...
        # (was only in run(), causing loomscan check and loomscan check --full to diverge)
        ...
        self._stage_complete(findings_count=len(result.findings))
        ...
        # v5.7: progress — stage 7 (auto-fix + reports)
        self._stage_start("AutoFix", "Generating fixes + saving reports",
                          animate_mascot=False)
        # Auto-fix
    ...
    def run(self, base: str = "HEAD", staged: bool = False) -> PipelineResult:
        ...
        # v5.7: progress — stage 2 (advanced engines)
        self._stage_start("Advanced", "Taint + CPG + typestate + metamorphic",
                          animate_mascot=False)
        n_before = len(result.findings)
        ...
        # Step 4h: filter suppressed findings (inline `# loomscan: ignore`)
        kept, suppressed = filter_suppressed(result.findings, self.repo_root)
        result.findings = kept
        result.suppressed_count = len(suppressed)
        result.suppressed_findings = suppressed  # v4.11: persist for audit
        ...
        # v5.7: progress — stage 3 (precision + counterfactual)
        self._stage_start("Precision", "FP learning + counterfactual mutation",
                          animate_mascot=False)
        ...
        # causing loomscan check and loomscan check --full to produce different results.
        result.findings += self._run_html_config_scan()
        result.findings += self._run_js_taint_tracking()
        result.findings += self._run_js_pattern_scan()
        result.findings += self._run_v2_analyzers()
        ...
        # This was one of 5 divergences between loomscan check and loomscan check --full.
        if self.project_tuner:
            for f in result.findings:
                mult = self.project_tuner.get_confidence_multiplier(f.rule_id)
                if mult != 1.0:
        ...
        # v5.7: progress — stage 5 (auto-fix + reports)
        self._stage_start("AutoFix", "Generating fixes + saving reports",
                          animate_mascot=False)
        fixable_findings = [f for f in result.findings if f.severity in (Severity.HIGH, Severity.CRITICAL)]
        if fixable_findings:
    ...
    def _run_hotspot_detection(self, hunks: List[DiffHunk]) -> List[Finding]:
        ...
        for h in new_hotspots:
            findings.append(Finding(
                ...
                blast_radius=BlastRadius.FUNCTION, exploitability=0.3,
                cwe="CWE-863",
                fix_suggestion=f"Review this hotspot: `loomscan hotspot review {h.id} safe|confirmed`",
        ...
        for h in decayed:
            findings.append(Finding(
                ...
                blast_radius=BlastRadius.FUNCTION, exploitability=0.2,
                cwe="CWE-863",
                fix_suggestion=f"Re-review: `loomscan hotspot review {h.id} safe|confirmed`",
    ...
    def _run_coverage_checks(self, hunks: List[DiffHunk]) -> List[Finding]:
        ...
        # track coverage history (writes to .loomscan-coverage-history.json)
