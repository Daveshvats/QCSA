"""v4.37: PR comment bot — post LoomScan findings as GitHub PR review comments.

Reads the GitHub Actions event payload, runs LoomScan on the PR diff, and
posts findings as inline review comments via the GitHub API.

Usage (in GitHub Actions):
    loomscan bot --repo $GITHUB_WORKSPACE --pr ${{ github.event.pull_request.number }} \
            --token ${{ secrets.GITHUB_TOKEN }} --event $GITHUB_EVENT_PATH

The bot:
  1. Parses the PR event payload to get the PR number, head SHA, and diff
  2. Runs `loomscan check` on the PR diff (diff-aware — only new findings)
...
def run_loomscan_check(repo_root: Path, base: str = "origin/main") -> List[Finding]:
    """Run `loomscan check --json` and parse findings."""
    env = os.environ.copy()
    ...
    if proc.returncode not in (0, 1):
        raise RuntimeError(f"loomscan check failed (exit {proc.returncode}): {proc.stderr}")
    try:
        data = json.loads(proc.stdout)
    except json.JSONDecodeError as e:
        raise RuntimeError(f"loomscan check returned invalid JSON: {e}") from e
    findings = []
...
def post_pr_review(
    ...
    for file, file_findings in by_file.items():
        for f in file_findings:
            comments.append({
                ...
                "line": f.line,
                "side": "RIGHT",
                "body": f"**LoomScan {f.severity.upper()}** — `{f.rule_id}`\n\n{f.message}\n\n_Confidence: {f.confidence:.0%}_",
            })
    ...
    if not findings:
        event = "APPROVE"
        body = "LoomScan found no issues in this PR."
    elif has_critical or has_high:
        event = "REQUEST_CHANGES"
        body = f"LoomScan found {len(findings)} issue(s) in this PR, including critical/high severity. Please address them before merging."
    else:
        event = "COMMENT"
        body = f"LoomScan found {len(findings)} issue(s) in this PR (medium/low severity)."
    ...
    if len(comments) > 50:
        body += f"\n\n_(Showing 50 of {len(comments)} inline comments — see full report in the LoomScan action run.)_"
...
def bot_cmd(repo: str, pr_number: Optional[int], token: Optional[str],
    ...
    Reads the PR diff, runs LoomScan, posts inline review comments via the GitHub API.

    Examples:
      loomscan bot --repo . --pr 42 --token $GITHUB_TOKEN
      loomscan bot --dry-run          # print without posting
    """
    ...
    click.echo(f"LoomScan bot — PR #{pr_num} in {repo_owner}/{repo_name}")
    click.echo(f"Running LoomScan check (base: {base_ref})...")

    try:
        findings = run_loomscan_check(repo_root, base=base_ref)
    except Exception as e:
        click.echo(f"Error: LoomScan check failed: {e}", err=True)
        sys.exit(2)
