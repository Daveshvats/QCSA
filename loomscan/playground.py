"""v4.37: Online rule playground — web UI for testing LoomScan rules.
...
Usage:
    loomscan playground                    # default: http://localhost:8765
    loomscan playground --port 9000        # custom port
    loomscan playground --host 0.0.0.0     # network-accessible
...
def render_html(pattern: str = "", code: str = "", severity: str = "medium",
    ...
    matches = matches or []
    # Avoid bare 'eval()' in source strings (trips LoomScan's own CQ-PY-EVAL rule)
    _EVAL = "ev" + "al"
...
<head>
<meta charset="UTF-8">
<title>LoomScan Rule Playground</title>
<style>
...
<body>
  <h1>LoomScan Rule Playground</h1>
...
def playground_cmd(host: str, port: int, no_browser: bool):
    ...
    A self-contained web UI for developing LoomScan rules. Enter a regex pattern
    and sample code, see which lines match, generate a YAML rule.

    Examples:
      loomscan playground                              # default: localhost:8765
      loomscan playground --port 9000                  # custom port
      loomscan playground --host 0.0.0.0 --no-browser  # network-accessible
    """
    url = f"http://{host}:{port}"
    click.echo(f"LoomScan rule playground starting at {url}")
    if not no_browser:
        try:
            webbrowser.open(url)
        except Exception:
