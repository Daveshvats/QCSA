"""Pre-commit hook for LoomScan — runs on staged files, < 3 second budget."""
...
def run_pre_commit(repo_root: Path, block_only: bool = True, time_budget_ms: int = 3000) -> int:
    ...
    print(f"LoomScan: scanning {len(source_files)} staged file(s)...", file=sys.stderr)
    findings: List[dict] = []
    try:
        ...
        print(f"LoomScan: analyzer error: {e}", file=sys.stderr)
    elapsed_ms = (time.time() - start) * 1000
    if not findings:
        print(f"LoomScan: no issues found ({elapsed_ms:.0f}ms)", file=sys.stderr)
        return 0
    ...
    if not block_findings:
        print(f"LoomScan: {len(findings)} non-blocking issue(s) found ({elapsed_ms:.0f}ms)", file=sys.stderr)
    ...
    print("To investigate: loomscan check <file>", file=sys.stderr)
    return 1

def main():
    parser = argparse.ArgumentParser(description="LoomScan pre-commit hook")
