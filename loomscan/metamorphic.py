...
def discover_testable_functions(file_path: Path) -> List[Tuple[str, str, List[str], int, List[str]]]:
    ...
    # v5.3: Multi-language function discovery
    ext = file_path.suffix.lower()
    if ext == ".py":
        return _discover_py_functions(file_path)
    elif ext in (".js", ".jsx", ".mjs"):
        return _discover_js_functions(file_path)
    elif ext == ".java":
        return _discover_java_functions(file_path)
    elif ext == ".go":
        return _discover_go_functions(file_path)
    else:
        return []


def _discover_py_functions(file_path: Path) -> List[Tuple[str, str, List[str], int, List[str]]]:
    """Discover testable functions in a Python file."""
    try:
...
def _discover_js_functions(file_path: Path) -> List[Tuple[str, str, List[str], int, List[str]]]:
    """v5.3: Discover testable functions in a JavaScript file using regex."""
    import re
    try:
        source = file_path.read_text(encoding="utf-8")
    except Exception:
    ...
    # Match: function name(args) { ... } OR const name = (args) => { ... } OR name(args) { ... }
    patterns = [
    ...
    for pat in patterns:
        for m in re.finditer(pat, source):
            ...
            params = [p.strip().split(":")[0].split("=")[0].strip() for p in params_str.split(",")] if params_str else []
            arity = len(params)
            if arity == 0 or arity > 2:
                continue
            # Extract function body (approximate — find the next { and matching })
            body_start = m.end()
            # Simple heuristic: body is the next 500 chars
            body = source[body_start:body_start + 500]
            cats = _classify_function(func_name, body, arity, ["any"] * arity)
            results.append((func_name, f"({params_str})", cats, arity, ["any"] * arity))

    return results


def _discover_java_functions(file_path: Path) -> List[Tuple[str, str, List[str], int, List[str]]]:
    """v5.3: Discover testable methods in a Java file using regex."""
    import re
    try:
        source = file_path.read_text(encoding="utf-8")
    except Exception:
    ...
    pat = r'(?:public|private|protected|static|final|synchronized|\s)+\s+\w+(?:<[^>]+>)?\s+(\w+)\s*\((([^)]*))\)\s*(?:throws\s+[\w.,\s]+)?\s*\{'
    for m in re.finditer(pat, source):
        func_name = m.group(1)
        if func_name.startswith("_") or func_name in ("main", "toString", "hashCode", "equals"):
            continue
        params_str = m.group(2).strip()
        params = [p.strip().split()[-1].split("=")[0].strip() for p in params_str.split(",")] if params_str else []
        arity = len(params)
        if arity == 0 or arity > 2:
            continue
        body_start = m.end()
        body = source[body_start:body_start + 500]
        cats = _classify_function(func_name, body, arity, ["Object"] * arity)
        results.append((func_name, f"({params_str})", cats, arity, ["Object"] * arity))

    return results


def _discover_go_functions(file_path: Path) -> List[Tuple[str, str, List[str], int, List[str]]]:
    """v5.3: Discover testable functions in a Go file using regex."""
    import re
    try:
        source = file_path.read_text(encoding="utf-8")
    except Exception:
    ...
    # Match: func name(params) returnType { OR func (recv) name(params) {
    pat = r'\bfunc\s+(?:\([^)]+\)\s+)?(\w+)\s*\((([^)]*))\)'
    for m in re.finditer(pat, source):
        func_name = m.group(1)
        if func_name.startswith("_") or func_name in ("main", "init", "TestMain"):
            continue
        params_str = m.group(2).strip()
        # Go params: "name type, name type"
        params = [p.strip().split()[-1] if p.strip() else "" for p in params_str.split(",")] if params_str else []
        arity = len(params)
        if arity == 0 or arity > 2:
            continue
        body_start = m.end()
        body = source[body_start:body_start + 500]
        cats = _classify_function(func_name, body, arity, ["interface{}"] * arity)
        results.append((func_name, f"({params_str})", cats, arity, ["interface{}"] * arity))
...
def run_metamorphic_tests(file_path: Path, repo_root: Path = None) -> List[MetamorphicViolation]:
    ...
    v5.3: For Python, generates and runs Hypothesis tests.
    For JS/Java/Go, performs static MR analysis (checks if the function
    body looks like it should satisfy certain MRs based on code patterns).
    """
    functions = discover_testable_functions(file_path)
    if not functions:
        return []

    # v5.3: For non-Python files, do static MR analysis
    ext = file_path.suffix.lower()
    if ext not in (".py",):
        return _static_mr_analysis(file_path, functions, repo_root)

    # Python: generate and run Hypothesis tests
    ...
    test_file = (repo_root or file_path.parent) / ".loomscan-cache" / "metamorphic" / f"test_{file_path.stem}_mr.py"
...
def _static_mr_analysis(file_path: Path,
                        functions: List[Tuple[str, str, List[str], int, List[str]]],
                        repo_root: Path = None) -> List[MetamorphicViolation]:
    """v5.3: Static metamorphic relation analysis for JS/Java/Go.

    For languages we can't execute (JS/Java/Go), we do a static analysis:
    check if the function body contains patterns that suggest MR violations.
    This is weaker than Python's execution-based testing but still catches
    some bugs (e.g., non-deterministic functions, missing idempotence).

    Checks:
    1. Determinism: function body should not use Date.now(), Math.random(),
       System.currentTimeMillis(), time.Now(), etc.
    2. Idempotence: if function name suggests idempotence (normalize, sanitize,
       trim, lowercase, etc.), verify no side effects.
    3. Commutativity: if function name suggests commutativity (add, sum,
       multiply, concat), verify both args are used symmetrically.
    """
    import re
    violations: List[MetamorphicViolation] = []
    rel_path = str(file_path.relative_to(repo_root)) if repo_root else str(file_path)
    ...
    # Non-determinism indicators by language
    nondet_patterns = {
        ".js": [r"\bMath\.random\s*\(", r"\bDate\.now\s*\(", r"\bDate\(\s*\)",
                r"\bprocess\.uptime\s*\(", r"\bperformance\.now\s*\("],
        ".java": [r"\bSystem\.currentTimeMillis\s*\(", r"\bMath\.random\s*\(",
                  r"\bnew\s+Random\s*\(", r"\bThreadLocalRandom\b",
                  r"\bUUID\.randomUUID\s*\("],
        ".go": [r"\btime\.Now\s*\(", r"\brand\.Intn\s*\(", r"\brand\.Float64\s*\(",
                r"\brand\.Int\s*\(", r"\bmath/rand\b"],
    }

    patterns = nondet_patterns.get(ext, [])
    lines = source.splitlines()

    for func_name, sig, cats, arity, type_hints in functions:
        # v5.3: Check ALL functions for non-determinism (not just "deterministic" category)
        # Any function using random/time sources is suspicious
        for i, line in enumerate(lines, 1):
            if func_name in line:
                body_lines = lines[i-1:min(i+19, len(lines))]
                body_text = "\n".join(body_lines)
                for pat in patterns:
                    if re.search(pat, body_text):
                        violations.append(MetamorphicViolation(
                            ...
                            description=f"Function '{func_name}' appears non-deterministic (uses random/time source)",
                            input_summary=f"Pattern: {pat}",
                            file=rel_path,
                        ))
                        break
                break
