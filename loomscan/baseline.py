...
code. The baseline solves this:
  ...
  1. `loomscan baseline create` — runs LoomScan on the current codebase, saves ALL
     finding fingerprints to .loomscan-baseline.json
  2. `loomscan check --baseline` — only flags findings whose fingerprint is NOT
     in the baseline (i.e., new issues)
  3. `loomscan baseline update` — adds current findings to the baseline (after
     you've reviewed/acknowledged them)

This enables incremental improvement: you don't have to fix 500 legacy issues
before LoomScan is useful. You baseline them, then LoomScan only blocks NEW issues.
...
BASELINE_FILE = ".loomscan-baseline.json"
