...
def spec_cmd(repo: str, max_files: int, mine_only: bool, check_only: bool, min_confidence: float):
    ...
    This is the next auto-derivation path after `loomscan mine`:
    - `loomscan mine` derives rules from bug-fix COMMITS
    - `loomscan spec` derives specs from correct USAGE patterns

    Examples:
      loomscan spec                              # mine + check (default)
      loomscan spec --mine-only                  # just mine patterns, show them
      loomscan spec --check-only                 # just check for violations
      loomscan spec --max-files 500              # scan more files
    """
